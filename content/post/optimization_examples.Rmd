...
For these examples, we will download the daily prices for a handfull of ETFs from Yahoo Finance, starting in 2003.  Once the daily data is downlaoded, we use the *to.weekly()* function to convert the prices to a weekly series (there is also a *to.monthly()* function) and then combine the series column by column into a data.frame.

```{r prices, warnings=FALSE, messages=FALSE, cache=TRUE}
...
Once we have the prices, we can use the *lag* function to calculate the (weekly) returns. We also create a logprices series just for good measure.
...
We show three different ways to calculate the same stndard deviation above.  The third one is essentially the same as the second, we just use the magrittr package in R which gives us the *%>%* operator that makes the code easier to read.  We will show you a fourth way to calculate standard deviation in the next section.
...
```{r srhink, warnings=FALSE, messages=FALSE}
library(corpcor)
...
plot_portfolio <- function(w, label, V, mu) {
  ...
  b <- c(1, rep(0,n))
  f <- function(u) {
    solve.QP(u * V, mu, A, b, meq = 1)$solution
  }
...
# $\lambda$ = 1: Neutral risk-averse investor
```{r longonly, warnings=FALSE, messages=FALSE}
r <- solve.QP(Dma=V, dvec=mu, Amat, bvec, meq=1)
sum(r$solution)
round(r$solution, 2)
...
# $\lambda$ = .1:  risk-loving investor
```{r longonly2, warnings=FALSE, messages=FALSE}
r <- solve.QP(Dma=.1 * V, dvec=mu, Amat, bvec, meq=1)
sum(r$solution)
round(r$solution, 2)
plot_portfolio(w=r$solution, label="lambda = .1", V, mu)
```
...
# $\lambda$ = 3: More risk-averse investor
```{r riskAverse1, warnings=FALSE, messages=FALSE}
r <- solve.QP(Dma=3 * V, dvec=mu, Amat, bvec, meq=1)
plot_portfolio(w=r$solution, label="lambda = 3", V, mu)
```

# $\lambda$ = 10: Very risk-averse investor
```{r riskAverse2, warnings=FALSE, messages=FALSE}
r <- solve.QP(Dma=10 * V, dvec=mu, Amat, bvec, meq=1)
plot_portfolio(w=r$solution, label="lambda = 10", V, mu)
```
